...
def run_simulation(todo):
    ...
    try:
        ...
        elif todo["conv_type"].startswith("embedded"):
            conversation_simulator = ConversationSimulatorEmbedded(todo["sample"], assistant_model=todo["assistant_model"], system_model=todo["system_model"], user_model=todo["user_model"], assistant_temperature=assistant_temp, user_temperature=user_temp, dataset_fn=dataset_fn, log_folder=args.log_folder)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--N_embedded_runs", type=int, default=1, help="Number of embedded (collapsed-to-one-turn) runs per model")
    ...
    embedded_ct = f"embedded{sharded_extra}"

    all_tasks = list(set([sample["task"] for sample in samples]))
    for assistant_model in args.models:
        sharded_run_counts, full_run_counts, concat_run_counts, embedded_run_counts = Counter(), Counter(), Counter(), Counter()
        for task in all_tasks:
            ...
            embedded_run_counts.update(get_run_counts(embedded_ct, task, assistant_model, dataset_fn, log_folder=args.log_folder))
        ...
        print(f"Embedded run counts: {embedded_run_counts}")
        ...
        for sample in samples:
            ...
            all_todos += [{"sample": sample, "assistant_model": assistant_model, "conv_type": embedded_ct, "system_model": args.system_model, "user_model": args.user_model, "dataset_fn": dataset_fn}] * (args.N_embedded_runs - embedded_run_counts[sample["task_id"]])
